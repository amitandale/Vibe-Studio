...
- **`tls error (server refused TLS connection)` or `could not open file "global/pg_filenode.map": Permission denied` during `supabase db push`**: Ensure the lane connection string ends with `?sslmode=disable` so the CLI negotiates plaintext connections to the local Postgres instance. The deploy workflow now repairs the Supabase database volume permissions and resets the lane superuser password automatically when it encounters this error. If the automated recovery still fails, run the following commands from the lane checkout as a manual fallback and re-run the workflow:
