...
if ! command -v "$python_bin" >/dev/null 2>&1; then
  if command -v python >/dev/null 2>&1; then
    ...
    echo "required command 'python3' (or 'python') not found in PATH" >&2
    exit 1
  fi
fi
    ...
    if command -v jq >/dev/null 2>&1; then
      ...
      if ps_json=$("${compose_cmd[@]}" ps --format json 2>/dev/null); then
        if [[ -z "$ps_json" ]]; then
          ...
          cleanup_envfiles+=("$json_err_file")
          services_json_file="$(mktemp)"
          cleanup_envfiles+=("$services_json_file")
          if ! PS_JSON="$ps_json" "$python_bin" <<'PY' >"$services_json_file" 2>"$json_err_file"; then
import json
import os
import sys

raw = os.environ.get("PS_JSON", "")


def append_services(node, dest):
    if isinstance(node, list):
        for item in node:
            append_services(item, dest)
    elif isinstance(node, dict):
        if "Services" in node:
            services = node.get("Services")
            if isinstance(services, dict):
                append_services(list(services.values()), dest)
            else:
                append_services(services, dest)
        else:
            dest.append(node)
...
def record_error(errors, exc, snippet):
    prefix = snippet[:200].replace("\n", " ")
    errors.append(f"invalid compose ps json line: {exc}: {prefix}")


def load_and_append(value):
    if value is None:
        return
    append_services(value, normalized)


text = raw.strip()
if not text:
    # empty payload -> no services
    print("[]")
    sys.exit(0)


def try_load_document(doc_text):
    try:
...
try:
    ...
    for line in raw.splitlines():
        ...
        try:
            ...
            record_error(errors, exc, stripped)
            continue
        load_and_append(entry)
else:
    load_and_append(value)
...
PY
          ...
          services_json="$(cat "$services_json_file")"
