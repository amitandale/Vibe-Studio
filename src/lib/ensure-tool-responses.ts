...
export function createEnsureToolResponses(uuidFactory: UuidFactory = uuidv4) {
  return function ensureToolCallsHaveResponses(messages: Message[]): Message[] {
    ...
    messages.forEach((message, index) => {
      ...
      newMessages.push(
        ...(message.tool_calls?.map((tc) => ({
          type: "tool" as const,
          tool_call_id: tc.id ?? "",
          id: `${DO_NOT_RENDER_ID_PREFIX}${uuidFactory()}`,
          name: tc.name,
...
export const ensureToolCallsHaveResponses = createEnsureToolResponses();
